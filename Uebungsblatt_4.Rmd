...
```{r "1_a"}
data <- read_csv("/Users/merttanriover/Downloads/data/EU_Makro.csv")
...
g1 <- eu_makro %>%
  ggplot(aes(x = debt_share_country_pro, y = unempl)) + 
  geom_point() + 
...
Wir sehen, dass die 5000 simulierten Schaetzer um den wahren Wert von beta1 verteilt sind. Folgender ggplot2 zeigt das noch etwas huebscher:
```{r "2_c_3"}
ggplot(mapping=aes(x=sim.beta1)) + 
...
```{r "3_a_3"}
dat <- eu_makro %>%
  filter(country == "Germany" | country == "France" | country == "Netherlands")
...
Wenn das 95% Konfidenzintervall eines Schaetzers nicht mehr die 0 enthaelt, können Sie von einem signifikanten Schaetzer zum Signifikanzniveau von 5% sprechen.
...
Die letzte Spalte `Pr(>|t|)` gibt den sog. p-Wert des Schaetzers an. Der p-Wert ist das hoechste Signifikanzniveau zu dem der Schaetzer noch signifikant ist. Hier haben wir einen p-Wert von 0.0505, d.h. 5.05%. Dies bedeutet, dass 94.5% Konfidenzintervall  endet rechts genau auf der 0.
